...
def login_action(request):
    if request.method == "POST":
        ...
        if login_username == '' or login_password == '':
            ...
            user = auth.authenticate(username = login_username, password = login_password) #验证用户名密码
            if user is not None:
...
def sign_index_action(request, event_id):
    ...
    if not result:
        return render(request, 'sign_index.html', {'event': event,'hint': '手机号不存在.','guest':guest_data,'sign':sign_data})

    result = Guest.objects.filter(phone = phone,event_id = event_id)
    if not result:
        return render(request, 'sign_index.html', {'event': event,'hint': '手机号与发布会id不匹配.','guest':guest_data,'sign':sign_data})

    result = Guest.objects.get(event_id = event_id,phone = phone)

    if result.sign:
        return render(request, 'sign_index.html', {'event': event,'hint': "用户已签到.",'guest':guest_data,'sign':sign_data})
    else:
